var util = require('util'),
    ...
    SuperColumn = require('./super_column'),
    Row = require('./row'),
...
/**
 * Gets an array of super columns from an object
 * @param {Object} columns
 * @private
 * @memberOf ColumnFamily
 * @returns {Array} and array of super columns
 */
function getSuperColumns(columns) {
  var keys = Object.keys(columns);
  var arr = new Array(keys.length);
  for(var i=0; i < keys.length; i++) {
    arr[i] = new SuperColumn(keys[i], columns[keys[i]]);
  }
  return arr;
}
 ...
 *
 * cf.insert("Locations", {
 *   "Boulder": {"Latitude": 40.015, "Longitude": -105.27},
 *   "Fort Collins": {"Latitude": 40.56, "Longitude": -105.07}
 * });
 *
 */
ColumnFamily.prototype.insert = function(key, columns, options, callback){
  ...
  if (this.isSuper) {
    if(!Array.isArray(columns)){
      columns = getSuperColumns(columns);
    }
    
    var subcolumnMarshaller = new Marshal(this.definition["subcomparator_type"]);
    var mutations = new Array(columns.length);
    var supercolumn;

    for(var i=0; i < columns.length; i++) {
      supercolumn = columns[i];
      mutations[i] = new ttype.Mutation({
        column_or_supercolumn: new ttype.ColumnOrSuperColumn({
          super_column: supercolumn.toThrift(this.columnMarshaller, subcolumnMarshaller, this.valueMarshaller)
        })
      });
    }
    ...
    var len = columns.length, i = 0, valueMarshaller, col, mutations = [],
